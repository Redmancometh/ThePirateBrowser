...
if not exist "%APP_ROOT%ThePirateBrowser.exe" goto incomplete
if not exist "%APP_ROOT%app\ThePirateBrowser.cfg" goto incomplete
if not exist "%APP_ROOT%app\the-pirate-browser.jar" goto incomplete
...
start "" "%APP_ROOT%ThePirateBrowser.exe"
exit /b 0

:incomplete
echo.
echo The Pirate Browser folder is incomplete.
echo Extract and keep the entire folder together, then run this launcher again.
echo Do not copy or run ThePirateBrowser.exe by itself.
echo.
